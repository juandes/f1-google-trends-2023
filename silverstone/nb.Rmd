...
```{r}
setwd("~/Development/f1-google-trends/silverstone")
df <- read.csv("~/Development/f1-google-trends/silverstone/data/multiTimeline.csv")
```
...
df$tzts <- as.POSIXct(df$Time, tz = "America/Chicago", format = "%Y-%m-%dT%H:%M:%S")
...
p <- ggplot(df, aes(x = tzts, y = interest)) +
  ...
  labs(title="Google Trends' Interest over time of the phrase \"formula 1\"",
       subtitle = "Queried on Sunday, May 29, 2023 at 15:12 UTC") +
